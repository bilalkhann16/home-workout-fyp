...
def conv2(inputs, filters, kernel_size, name, padding='SAME', act=True, normalization=True):
    ...
    with tf.compat.v1.variable_scope(name) as scope:
        ...
        w = tf.compat.v1.get_variable('weights', shape=[kernel_size, kernel_size, channels_in, filters], trainable=True,
                            initializer=tf.compat.v1.keras.initializers.VarianceScaling(scale=1.0, mode="fan_avg", distribution="uniform"))
        # b = tf.Variable(tf.truncated_normal(shape=[filters], stddev=0.1))
        b = tf.compat.v1.get_variable('biases', shape=[filters], trainable=True,
                            initializer=tf.compat.v1.keras.initializers.VarianceScaling(scale=1.0, mode="fan_avg", distribution="uniform"))
...
def gen_network(input):
    cpm_out = []
    hm_out = []
    with tf.compat.v1.variable_scope('train_layers'):
