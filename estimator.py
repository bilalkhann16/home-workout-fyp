    ...
    def estimate_paf(peaks, heat_mat, paf_mat):
        ...
        for human_id in range(pafprocess.get_num_humans()):
            ...
            for part_idx in range(18):
                ...
                # print ("human.body_parts[part_idx]", human.body_parts[part_idx])
...
class TfPoseEstimator:
    ...
    def draw_humans(npimg, humans, imgcopy=False):
        ...
        for human in humans:
            ...
            for i in range(common.CocoPart.Background.value):
                ...
                # print ("human.body_parts[i]", body_part," ",i)
                # print("\n\n bodyparts", body_part.x, body_part.y)
                center = (int(body_part.x * image_w + 0.5), int(body_part.y * image_h + 0.5))
                # print ("center", center)
                centers[i] = center
                cv2.circle(npimg, center, 3, common.CocoColors[i], thickness=5, lineType=8, shift=0)  #circle on key points currently being detected by the network

            # draw line
            
            print("common.CocoPairsRender", common.CocoPairsRender)  #the eypoints of the person defined in common.py
            print ("human.body_parts.keys()", human.body_parts)   #keypoints currently being detected by the network and webcam (the dots.)
            print ("centers", centers)
            
            for pair_order, pair in enumerate(common.CocoPairsRender):
                print ("pair_order, pair", pair_order,pair)
                print ("pair[0], pair[1]: ",len(pair), pair[0], pair[1])
                
                if pair[0] not in human.body_parts.keys() or pair[1] not in human.body_parts.keys():
                    print (pair[0], pair[1], "not in ", human.body_parts.keys())
                    continue
                # if pair[0] not in pairs_1 or pair[1] not in pairs_1:
                #     print ("not in pair", pair[0],pair[1])
                #     continue

                # npimg = cv2.line(npimg, centers[pair[0]], centers[pair[1]], common.CocoColors[pair_order], 3)
                
                # print ("we rolling", centers[pair[0]])
                # print ("common.CocoColors", common.CocoColors)
                
                try:
                    cv2.circle(npimg, centers[5], 10, [0,0,0], thickness=8, lineType=8, shift=0)
                    cv2.circle(npimg, centers[6], 3, [0,0,0], thickness=8, lineType=8, shift=0)
                    cv2.circle(npimg, centers[7], 3, [0,0,0], thickness=8, lineType=8, shift=0)
                    
                    cv2.circle(npimg, centers[2], 10, [0,0,0], thickness=8, lineType=8, shift=0)
                    cv2.circle(npimg, centers[3], 3, [0,0,0], thickness=8, lineType=8, shift=0)
                    cv2.circle(npimg, centers[4], 3, [0,0,0], thickness=8, lineType=8, shift=0)
                    
                    #right.
                    body_part_1 = human.body_parts[5]
                    body_part_2 = human.body_parts[6]
                    body_part_3 = human.body_parts[7]
                    
                    #left
                    r_body_part_1 = human.body_parts[2]
                    r_body_part_2 = human.body_parts[3]
                    r_body_part_3 = human.body_parts[4]
                    
                    
                    shoulder_press_angle = math.degrees(math.atan2(body_part_3.y-body_part_2.y, body_part_3.x-body_part_2.x) 
                           - math.atan2(body_part_2.y-body_part_1.y, body_part_2.x-body_part_1.x))
                    shoulder_press_angle = -1 * shoulder_press_angle
                    ...
                    pass
                print ("\n Cuurent count", TfPoseEstimator.shoulder_press_count)
                print ("drawing line", pair[0], pair[1])
                cv2.line(npimg, centers[pair[0]], centers[pair[1]], common.CocoColors[pair_order], 3)
                cv2.putText(npimg, 'count'+ str(TfPoseEstimator.shoulder_press_count), (15, 50), cv2.FONT_HERSHEY_COMPLEX_SMALL, 1, (0,0,0))
                # cv2.line(npimg, centers[pair[5]], centers[pair[16]], [0,0,0], 10)
                # cv2.line(npimg, centers[0], centers[1], [255, 0, 0], 3) 
                # time.sleep(5)               
        return npimg
    ...
    def inference(self, npimg, resize_to_default=True, upsample_size=1.0):
        ...
        if resize_to_default:
            upsample_size = [int(240 / 8 * upsample_size), int(320 / 8 * upsample_size)]
        else:
        ...
        t = time.time()
        print ("line 614 in estimator.py")
