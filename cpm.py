...
class PafNet:
    ...
    def conv2(self, inputs, filters, padding, kernel_size, name, act=True, normalization=False):
        ...
        with tf.compat.v1.variable_scope(name) as scope:
            w = tf.compat.v1.get_variable('weights', shape=[kernel_size, kernel_size, channels_in, filters], trainable=True, initializer=tf.compat.v1.keras.initializers.VarianceScaling(scale=1.0, mode="fan_avg", distribution="uniform"))
            b = tf.compat.v1.get_variable('biases', shape=[filters], trainable=True, initializer=tf.compat.v1.keras.initializers.VarianceScaling(scale=1.0, mode="fan_avg", distribution="uniform"))
            conv = tf.nn.conv2d(input=inputs, filters=w, strides=[1, 1, 1, 1], padding=padding)
            output = tf.nn.bias_add(conv, b)
            if normalization:
                axis = list(range(len(output.get_shape()) - 1))
                mean, variance = tf.nn.moments(x=conv, axes=axis)
    ...
    def gen_net(self):
        paf_pre = []
        hm_pre = []
        with tf.compat.v1.variable_scope('openpose_layers'):
            with tf.compat.v1.variable_scope('cpm_layers'):
            ...
            with tf.compat.v1.variable_scope('staget'):
                for i in range(self.stage_num - 1):
                    ...
                    if i < self.stage_num - 2:
                        net = tf.concat([hm_net, paf_net, added_layers_out], 3)   #concatenation
